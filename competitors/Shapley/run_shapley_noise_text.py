...
QA row it:
  ...
  2. For each noise level, inserts foreign chunks from all the graph.
...
Run with CWD = code/ so relative dataset paths
resolve, e.g.:
  cd code && ../.venv/bin/python ../competitors/Shapley/run_shapley_noise_text.py \
      ...
      --noise-percentages 0.1,0.2,0.3,0.5
"""
import os
import sys
...
def load_all_graph_chunks(dataset: str) -> list[str]:
    path = os.path.join(WORKING_DIRS[dataset], "kv_store_text_chunks.json")
...
async def run_noise(args, rag, rag_counter, hf_model, hf_tok, data):
    ...
    row_players, row_answer, row_meta = {}, {}, {}
    retrieved_pool, seen = [], set()
    for _, row in tqdm(data.iterrows(), desc="retrieve+pool", total=len(data)):
        ...
        for c in players:
            if c not in seen:
                seen.add(c)
                retrieved_pool.append(c)

    rids = list(row_players.keys())

    all_chunks = load_all_graph_chunks(args.dataset)
    pool = list(dict.fromkeys(split_into_players(all_chunks, args.granularity)))
    print(f"Foreign-{args.granularity} pool: {len(pool)} unique {args.granularity}s "
              f"from all {len(all_chunks)} KG chunks.")
