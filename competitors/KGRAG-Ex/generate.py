...
from code.src.query import *
from code.src.retrieve import *
from code.src.parser import *
from code.src.llm_judge import judge_response
from perturbations import delete_node, delete_edge
from code.src.counterfactuals.utils import compute_answer_similarity, cosine_similarity_norm
...
async def find_breaking_nodes_counterfactuals(
    ...
    llm_calls += await apply_breaking_node_perturbations(
        rag,
        question=question,
...
async def find_breaking_edges_counterfactuals(
    ...
    llm_calls += await apply_breaking_edge_perturbations(
        rag,
        question=question,
        original_answer=original_answer,
        ground_truth=ground_truth,
...
async def find_corrective_nodes_counterfactuals(
    ...
    llm_calls += await apply_corrective_node_perturbations(
        rag,
        question=question,
        original_answer=original_answer,
        ground_truth=ground_truth,
...
async def find_corrective_edges_counterfactuals(
    ...
    llm_calls += await apply_corrective_edge_perturbations(
        rag,
        question=question,
        original_answer=original_answer,
        ground_truth=ground_truth,
...
def save_operations_to_json(
    ...
    original_subgraph, 
    perturbed_subgraph, 
    output_dir: str = "competitors/KGRAG-Ex/results", 
    filename: str = None, 
    found: bool = True, 
    llm_calls: int = 0, 
    current_ops: list=[],
