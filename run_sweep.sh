...
# This script lives at the repo root, which is also the uv project root.
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Run with cwd = code/ so the python modules can resolve relative paths
# (qa/, benchmark/, src/embeddings/...). uv still uses the root pyproject.
