...
# Register the 2wiki dataset for this entrypoint (assets live under
# KGs/lightrag/2wiki + datasets/2wiki, mirroring synthetic/hotpotqa). WORKING_DIRS
# and DATASETS are the shared mutable objects from dataset_setup, so this also
# satisfies setup_dataset()'s membership check and the --dataset arg choices,
# without editing shared dataset_setup.py.
if "2wiki" not in WORKING_DIRS:
    WORKING_DIRS["2wiki"] = "KGs/lightrag/2wiki"
    DATASETS.append("2wiki")
...
# Ablation toggles (overridden by main() from CLI flags). Defaults reproduce the
# non-ablated reference setting.
USE_CACHE: bool = True       # f-output cache M (state-dedup memoization).
RECOMPUTE_EMB: bool = False  # I_E ablation: recompute node embeddings on the fly.
G = None
type_index = None
node_index = node_records = node_embeddings = node_lookup = None
edge_index = edge_records = edge_embeddings = edge_lookup = None
# Entity-name -> source content map (vdb_entities.json), loaded only for the I_E
# ablation so recomputed node embeddings match the precomputed store exactly.
node_content: dict = {}
...
def setup_dataset(name: str):
    ...
    # I_E ablation: load the exact entity content the precomputed index was built
    # from (KGs/lightrag/<ds>/vdb_entities.json :: content), keyed by entity_name
    # to match node_lookup / graph node ids. Embedding this with the same model
    # reproduces node_embeddings, so the ablation isolates time, not behavior.
    global node_content
    node_content = {}
    if RECOMPUTE_EMB:
        vdb_path = os.path.join(WORKING_DIRS[name], "vdb_entities.json")
        with open(vdb_path, "r", encoding="utf-8") as fh:
            vdb = json.load(fh)
        node_content = {
            item["entity_name"]: item.get("content", "")
            for item in vdb["data"] if "entity_name" in item
        }
...
def _node_text(node) -> str:
    """Source text for recomputing a node embedding (I_E ablation).

    Uses the exact entity content the precomputed index was built from
    (vdb_entities.json :: content), so recomputed similarities match the
    precomputed store and the ablation isolates time. Falls back to the in-graph
    name + description if the entity is absent from the content map.
    """
    if node in node_content:
        return node_content[node]
    attrs = G.nodes[node] if (G is not None and node in G.nodes) else {}
    desc = attrs.get("description", "") or ""
    return f"{node} {desc}".strip()


async def create_node_similarity_index(nodes, query_embedding):
    nodes = list(nodes)

    if RECOMPUTE_EMB:
        # I_E ablation: recompute node embeddings on the fly instead of reading
        # the precomputed store. Batched, mirroring create_edge_similarity_index,
        # so the index_time timer captures the recomputation cost.
        node_similarity_index = {}
        texts = [_node_text(node) for node in nodes]
        embeddings = await sentence_transformer_embed(texts) if texts else []
        for node, embedding in zip(nodes, embeddings):
            if embedding is not None:
                node_similarity_index[node] = cosine_similarity_norm(query_embedding, embedding)
            else:
                node_similarity_index[node] = 0.0
...
async def psp_probe(
    ...
    Flip criterion is parameterized so the same probe serves both directions:
      - breaking (T→F, default): reference=original_answer, hit_score=0
        (a "hit" is the judge rejecting the original answer).
      - corrective (F→T): reference=ground_truth, hit_score=1
        (a "hit" is the judge accepting the ground truth).
    ...
    """
    judge_reference = original_answer if reference is None else reference
    ...
    for v in pivots:
        ...
        score = await judge_response(question, new_response, judge_reference)
        psp_llm_time += time.perf_counter() - _t0

        llm_calls += 1
        flipped = (score == hit_score)
...
async def find_breaking_counterfactuals(
    ...
    node_similarity_index = await create_node_similarity_index(set(G.nodes), query_embedding)
    ...
    while Q:
        ...
        # f-output cache M: skip states already evaluated. Ablated by --no-cache,
        # which forces re-evaluation/re-expansion of duplicate states (bounded by
        # --max-llm-calls), isolating the cache's effect on LLM calls and time.
        if USE_CACHE:
            if state in state_cache:
                continue
            state_cache.add(state)
...
async def find_corrective_counterfactuals(
    ...
    ### PSP (corrective F→T): prune provably-unhelpful deletion stars and seed an
    ### upper-bound incumbent. Unlike breaking (deletion-only), corrective search
    ### includes additions, which may be a cheaper correction — so we do NOT
    ### short-circuit on a hit. The probe only reduces the deletion search space
    ### (via prune sets passed to expand) and supplies a cost bound; the main
    ### Dijkstra, which also explores additions, stays authoritative.
    psp_pruned_nodes: set = set()
    psp_pruned_edges: set = set()
    psp_response_cache: dict = {}
    incumbent_ops = None
    ...
    if use_psp and "delete_node" in current_ops:
        (psp_hits, psp_pruned_nodes, psp_pruned_edges,
         _probed_pivots, psp_response_cache, psp_calls, psp_llm_time) = await psp_probe(
            ...
            K=psp_k,
            unit_cost=unit_cost,
            reference=ground_truth,   # corrective flip = answer now matches GT
            hit_score=1,
        )
        llm_calls += psp_calls
        llm_time  += psp_llm_time
        ...
        if psp_hits:
            # Cheapest flipping pivot deletion is a valid feasible CFE → use it as
            # an upper-bound incumbent (and a fallback if the budget runs out).
            v, hit_cost, perturbed_cg, response = min(psp_hits, key=lambda h: h[1])
            incumbent_ops = [("delete_node", v)]
            incumbent_cg = perturbed_cg
            incumbent_cost = float(hit_cost)
            incumbent_response = response
            print(f"[PSP] corrective hits={len(psp_hits)} → incumbent cost={incumbent_cost} pivot={v}")

    # The incumbent caps the search: candidates costlier than it cannot improve.
    effective_max_cost = min(max_cost, incumbent_cost)
    ...
    while Q:
        ...
        if cost > effective_max_cost:
            print(f"Max cost {effective_max_cost} exceeded (current cost: {cost:.4f}). Stopping search.")
            break
        elif llm_calls > max_llm_calls:
            print(f"Max LLM calls {max_llm_calls} exceeded. Stopping search.")
            break
        ...
        if len(ops) > 0:
            ...
            # Reuse the PSP probe's f-output for states it already evaluated
            # (single-pivot deletions, cached by psp_probe under the same state
            # schema); pay/count a generation call only on a cache miss. Mirrors
            # the breaking path's psp_refine_star reuse.
            cached = psp_response_cache.pop(state, None)
            if cached is not None:
                new_response = cached
            else:
                llm_calls += 1
            ...
            if score == 1:
                # Dijkstra pops in cost order ⇒ the first flip is the min-cost
                # CFE over the (pruned) search space. Record and stop; the final
                # save below compares it against the PSP incumbent.
                found_ops = ops
                found_cg = cg
                found_cost = cost
                found_response = new_response
                print(f"Counterfactual Operations: {ops}")
                break
    ...
    ### Choose the cheaper of {main-search result, PSP incumbent}.
    if found_ops is not None and found_cost <= incumbent_cost:
        best_ops, best_cg, best_cost, best_response = (
            found_ops, found_cg, found_cost, found_response)
    elif incumbent_ops is not None:
        best_ops, best_cg, best_cost, best_response = (
            incumbent_ops, incumbent_cg, incumbent_cost, incumbent_response)
    else:
        best_ops, best_cg, best_response = None, None, None
        best_cost = last_cost

    if best_ops is not None:
        _t0 = time.perf_counter()
        answer_similarity = await compute_answer_similarity(ground_truth, best_response)
        llm_time += time.perf_counter() - _t0
        ...
        print(f"Counterfactual Operations: {best_ops}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # --- Ablation toggles (Section 6.4.5 / Table tab:ablation-psp) ---
    p.add_argument("--no-cache", action="store_true",
                   help="Ablation: disable the f-output cache M (state-dedup memoization).")
    p.add_argument("--no-ie", action="store_true",
                   help="Ablation: disable embedding index I_E; recompute node embeddings on the fly.")
    p.add_argument("--gen-llm", default=None,
                   help="Ablation: override the generation LLM (HF model id) for this run.")
    return p
...
async def main(args: argparse.Namespace):
    global adm, USE_CACHE, RECOMPUTE_EMB

    # Ablation toggles from CLI (defaults reproduce the reference setting).
    USE_CACHE = not args.no_cache
    RECOMPUTE_EMB = args.no_ie
    if args.gen_llm:
        # Swap the generation LLM by setting the module global read lazily by
        # get_llm(); retrieve.py binds the function, not the model id, so this
        # takes effect without editing llm/utils.py or retrieve.py. Must precede
        # initialize_lightrag() / the first generation call below.
        import src.llm.utils as _llm_utils
        _llm_utils.VLLM_MODEL = args.gen_llm
        print(f"[ablation] generation LLM overridden -> {args.gen_llm}")
    print(f"[ablation] cache M={'on' if USE_CACHE else 'OFF'} | "
          f"I_E={'recompute' if RECOMPUTE_EMB else 'precomputed'}")
