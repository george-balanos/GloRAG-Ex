...
def delete_node(context_graph: nx.Graph, node_to_delete: str):
    """Delete a node and sweep any degree-0 isolates left behind.

    F3 is relaxed: C' may become disconnected. Singleton cleanup runs over the
    whole graph (not just direct neighbors) so multi-step deletions that leave
    stray isolates further away are still cleaned up. For a single deletion
    this is equivalent to a neighbor-only sweep — only direct neighbors of the
    removed node can newly hit degree 0.
    """
    G = context_graph.copy()
    G.remove_node(node_to_delete)
...
def delete_edge(context_graph: nx.Graph, edge_to_delete: tuple):
    """Delete an edge and sweep any degree-0 isolates left behind. See
    delete_node for the singleton-sweep rationale under relaxed F3.
    """
    G = context_graph.copy()
