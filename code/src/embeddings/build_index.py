...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="build_index",
        description="Build HNSW node/edge indices from a LightRAG working directory.",
    )
    p.add_argument("--dataset", choices=DATASETS, default="synthetic",
                   help="Dataset name; reads KGs/lightrag/<dataset>/vdb_*.json, writes src/embeddings/<dataset>/{node,edge}_index.*")
    return p
